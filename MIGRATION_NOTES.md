...
Trade-off: normal proxied protected-media requests already read current channel state. Direct signed media requests now add that indexed channel lookup instead of trusting only the capability expiry, increasing D1 reads in exchange for immediate passcode/deletion revocation while still avoiding Vercel media transfer. A copy previously stored in an authorized browser’s private cache may remain reusable for up to five minutes, but fresh requests and revalidation are denied immediately.
