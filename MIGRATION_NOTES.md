...
### Frontend CSP permits the Toss SDK script — 2026-08-21

- `script-src` now allows only the official `https://js.tosspayments.com` origin required by `@tosspayments/tosspayments-sdk`.
- No wildcard Toss domains were added to frame or connection policies. Any additional payment-window requirement must be added from an observed browser violation and narrowed to the documented origin.

Deployment note: no database, Worker or R2 change is required. Redeploy the frontend because CSP headers are generated by Next.js.
