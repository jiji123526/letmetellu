...
export function HeaderMenu({ anchorRect, onSettings, onGallery, onLinks, onRules, onAdmin, onClose }: HeaderMenuProps) {
  const { t } = useLocale();
  ...
  const items = [
    ...
    ...(onRules ? [{ key: "rules", label: t("rules"), icon: `<svg viewBox="0 0 24 24" width="16" height="16"><circle cx="12" cy="12" r="10" fill="none" stroke="currentColor" stroke-width="2"/><path d="M12 16v-4M12 8h.01" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"/></svg>` }] : []),
