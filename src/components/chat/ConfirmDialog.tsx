...
export function ConfirmDialog({ title, message, confirmLabel, confirmColor, onConfirm, onCancel, showCancel = true, closeOnBackdrop = true }: ConfirmDialogProps) {
  const { t } = useLocale();
  return (
    <div
      className="fixed inset-0 z-[300] flex items-center justify-center"
      style={{ background: "rgba(0,0,0,.4)", padding: "24px" }}
      onClick={(e) => { if (closeOnBackdrop && e.target === e.currentTarget) onCancel(); }}
    >
      <div style={{ width: "100%", maxWidth: "300px", background: "var(--bg, #fff)", borderRadius: "18px", padding: "22px 20px 18px", boxShadow: "0 18px 50px rgba(0,0,0,.22)", textAlign: "center" }}>
        ...
        <div style={{ display: "flex", gap: "8px" }}>
          {showCancel && <button style={{ flex: 1, border: "none", borderRadius: "12px", padding: "12px", fontSize: "calc(var(--bubble-font-size, 16px) - 2px)", cursor: "pointer", fontFamily: "inherit", lineHeight: 1, background: "var(--card, #f2f2f7)", color: "var(--secondary-text, #3c3c43)" }} onClick={onCancel}>{t("cancel")}</button>}
