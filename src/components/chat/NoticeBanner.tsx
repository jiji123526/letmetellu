    ...
    <div style={{ position: "absolute", top: "12px", left: "12px", right: "12px", zIndex: 12, pointerEvents: "none" }}>
      ...
      >
        ...
        {canExpand ? (
          <button
            type="button"
            style={{ flex: 1, minWidth: 0, background: "none", border: "none", padding: 0, cursor: "pointer", textAlign: "left", color: "inherit", display: "flex", alignItems: "flex-start", gap: "0" }}
            onClick={() => setExpanded((value) => !value)}
            aria-expanded={expanded}
          >
            <span style={{ flex: 1, minWidth: 0, paddingRight: "4px", fontSize: "calc(var(--bubble-font-size, 14px) - 1px)", fontWeight: 500, color: "var(--notice-text)", lineHeight: 1.35, whiteSpace: "normal", overflowWrap: "anywhere" }}>
              {title}
            </span>
            <span style={{ ...controlStyle, marginTop: "1px" }}>
          ...
          <span style={{ flex: 1, minWidth: 0, fontSize: "calc(var(--bubble-font-size, 14px) - 1px)", fontWeight: 500, color: "var(--notice-text)", lineHeight: 1.35, whiteSpace: "normal", overflowWrap: "anywhere" }}>
            {title}
          </span>
        )}

        <button type="button" style={{ background: "none", border: "none", cursor: "pointer", marginTop: "1px", ...controlStyle }} onClick={handleDismiss}>
          ✕
        </button>
