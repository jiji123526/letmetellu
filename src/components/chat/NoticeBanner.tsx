...
export function NoticeBanner({ channelId, notice, onDismiss }: NoticeBannerProps) {
  ...
  const iconContainerSize = "calc(var(--bubble-font-size, 17px) + 7px)";
  const iconGlyphSize = "calc(var(--bubble-font-size, 17px) + 1px)";
  ...
  return (
    <div style={{ position: "absolute", top: "12px", left: "12px", right: "12px", zIndex: 12, pointerEvents: "none" }}>
      ...
      >
        <span style={{ width: iconContainerSize, height: iconContainerSize, marginRight: "8px", flexShrink: 0, display: "flex", alignItems: "center", justifyContent: "center", color: "var(--notice-text)", background: "color-mix(in srgb, var(--notice-text) 8%, transparent)", borderRadius: "999px" }}>
          <svg viewBox="0 0 32 32" style={{ width: iconGlyphSize, height: iconGlyphSize }} fill="currentColor" aria-hidden="true">
        ...
        <div style={{ flex: 1, minWidth: 0, display: "flex", alignItems: expanded ? "flex-start" : "center" }}>
          <span style={{ flex: 1, minWidth: 0, paddingTop: expanded ? "2px" : 0, paddingRight: canExpand ? "6px" : "2px", fontSize: "calc(var(--bubble-font-size, 14px) - 1px)", fontWeight: 500, color: "var(--notice-text)", lineHeight: expanded ? 1.35 : 1.15, whiteSpace: "normal", overflowWrap: "anywhere" }}>
            {title}
          </span>
          {canExpand && (
            <button
            ...
            >
              <svg viewBox="0 0 16 16" style={{ width: iconGlyphSize, height: iconGlyphSize, transform: expanded ? "rotate(180deg)" : "none" }} fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true">
                <path d="m2.5 4 5.5 7 5.5-7" />
              </svg>
            </button>
          )}
          <button type="button" style={controlButtonStyle} onClick={handleDismiss} aria-label="Dismiss notice">
            <svg viewBox="0 0 16 16" style={{ width: iconGlyphSize, height: iconGlyphSize }} fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" aria-hidden="true">
