...
export function NoticeBanner({ channelId, notice, onDismiss }: NoticeBannerProps) {
  ...
  return (
    <div style={{ margin: "8px", padding: "10px 14px", display: "flex", alignItems: "center", gap: "8px", flexWrap: "wrap", background: "var(--notice-bg)", border: "1px solid var(--hairline)", borderRadius: "14px", zIndex: 10, flexShrink: 0, boxShadow: "0 1px 4px rgba(0,0,0,.1)", lineHeight: 1 }}>
      {/* Megaphone icon */}
      <span style={{ flexShrink: 0, display: "flex", alignItems: "center", color: "var(--notice-text)", padding: "0 4px" }}>
      ...
      {/* Title */}
      <span style={{ flex: 1, fontSize: "var(--bubble-font-size, 14px)", fontWeight: 400, color: "var(--notice-text)", lineHeight: 1.4, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: expanded ? "normal" : "nowrap" }}>
        {title}
      </span>

      {/* Expand button (if body exists) */}
      {body && (
        <button style={{ background: "none", border: "none", cursor: "pointer", color: "var(--notice-meta)", fontSize: "var(--bubble-font-size, 16px)", padding: "0 4px", flexShrink: 0, lineHeight: 1 }} onClick={() => setExpanded(!expanded)}>
          {expanded ? "▲" : "▼"}
        </button>
      )}

      {/* Close button */}
      <button style={{ background: "none", border: "none", cursor: "pointer", color: "var(--notice-meta)", fontSize: "var(--bubble-font-size, 16px)", padding: "0 4px", lineHeight: 1 }} onClick={handleDismiss}>
        ✕
      </button>

      {/* Expanded body */}
      {body && expanded && (
        <div style={{ width: "100%", fontSize: "calc(var(--bubble-font-size, 14px) - 1px)", color: "var(--notice-meta)", lineHeight: 1.5, padding: "8px 0 0 36px", whiteSpace: "pre-wrap", wordBreak: "break-word" }}>
          {body}
        </div>
      )}
    </div>
