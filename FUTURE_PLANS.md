...
- Add a lightweight Worker socket-auth endpoint so `/api/ws-token` no longer calls full `/api/init` on every socket open or reconnect just to decide admin, reports-owner or room-viewer auth mode.
- Route owner-channel popup reads through the existing same-origin `/api/user` proxy instead of fetching the Worker directly from the browser, so preview and production keep one request path and avoid CORS-specific behavior.
- Revisit reply-parent recovery if profiling still shows history-thread churn after the bootstrap and reconnect work settles. The current parent lookup path can issue one `message-context` request per missing parent and then re-sort the mounted message window after each resolution.
