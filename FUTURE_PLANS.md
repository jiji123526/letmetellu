...
- D1 top-read review completed on 2026-08-14: the recursive 15-minute bucket query is a manual health-baseline audit and needs no hot-path optimization. Substring message search read `21.48k` rows across 14 executions, but remained `0.13%` of runtime at `1.4 ms` p50 and `1.6 ms` p99. Keep the current bounded search until production p99 approaches `50-100 ms` or usage grows materially; a full-text index would add migration, write, storage and synchronization costs that are not currently justified.
