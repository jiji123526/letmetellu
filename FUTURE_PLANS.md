...
- Flat reply rollout phase 1 completed on 2026-08-09: all newly submitted replies now resolve and store the existing top-level `reply_to` id. Next phases are a verified legacy-chain backfill, direct indexed root/child page expansion, removal of recursive compatibility reads, and a post-deploy D1 Insights comparison. The product intentionally does not retain the originally clicked child reply.
