...
- Flat reply rollout phase 1 completed on 2026-08-09: all newly submitted replies now resolve and store the existing top-level `reply_to` id. The production audit then found all 747 existing replies already flat, so the legacy-chain backfill was safely skipped. Next phases are direct indexed root/child page expansion, removal of recursive compatibility reads, and a post-deploy D1 Insights comparison. The product intentionally does not retain the originally clicked child reply.
