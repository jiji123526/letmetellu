...
- Flat reply rollout phases 1–3 completed on 2026-08-09: new replies store the top-level `reply_to`, the production audit found all 747 existing replies already flat, and normal message pages now expand roots and indexed direct children without recursion. Remaining work is converting the lower-frequency `message-context` compatibility path and comparing the new direct query against the former ~97k average rows-read recursive query after representative traffic. The product intentionally does not retain the originally clicked child reply.
