...
- Measurement follow-up started on 2026-08-09: recursive root-thread expansion now reuses one visible deleted-parent set per query. Compare the new query's average rows read and duration against the prior query in D1 Insights before considering a persisted `thread_root_id` migration or a more invasive paging-contract change.
- Flat reply rollout phases 1–4 completed on 2026-08-09: new replies store the top-level `reply_to`, the production audit found all 747 existing replies already flat, and both normal message paging and `message-context` now expand roots and indexed direct children without recursion. Remaining work is operational verification: rerun the flat-reply audit after representative traffic and compare the new direct queries against the former ~97k average rows-read recursive query in D1 Insights. The product intentionally does not retain the originally clicked child reply.
