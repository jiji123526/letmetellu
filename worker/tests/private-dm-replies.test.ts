...
function createFakeEnv(options: {
  ...
  function statement(sqlText: string, params: unknown[] = []) {
    ...
    return {
      ...
      async all() {
        if (
          sql.includes("SELECT id, client_message_id, uid, auth_uid, nick, text, image")
...
test("sender DM roots expose deletion without unsupported reaction controls", () => {
  ...
  assert.match(messageListSource, /\{!msg\.dm && \(\s*<div data-message-swipe-content>\s*<ReactionBadge/);
