...
| `/internal/d1-canary/copy-verify` | Hidden | Hidden | Hidden | Hidden from browser/admin sessions | Dedicated operator-secret boundary; GET-only, reports channel excluded, aggregate actor/derived counts and fixed blocker codes only, no mutation, repair, browser proxy, or CORS access |
...
| `/internal/d1-canary/dispatch-once` | Hidden | Hidden | Hidden | Hidden from browser/admin sessions | Separate 32+ character dispatch secret, write-maintenance required, scheduled dispatcher disabled, exact POST command; claims at most ten events for one explicit canary shard, channel, and `channel_report` aggregate, returning counts only with no browser proxy or CORS access |
