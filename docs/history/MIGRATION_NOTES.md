...
- The capability also carries an allowlisted, signed channel presentation snapshot. Re-entry and in-channel refresh can reuse it to skip the same slow first channel lookup; a fast-path init never renews the token, so the original 30-second or two-minute authority window cannot be extended indefinitely without a fresh D1 read. Oversized snapshots are not issued as cookies and safely retain the authoritative read path.
